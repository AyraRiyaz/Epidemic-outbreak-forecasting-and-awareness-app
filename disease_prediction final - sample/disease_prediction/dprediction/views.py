...
def main(request):
    ...
    import csv
    # Open CSV file
    # with open(r"C:\Users\kpriy\Downloads\malaria_sample_data.csv", 'r') as file:
...
def login(request):
    ...
    try:
        ...
        print("Session Set:", request.session["lid"])
        if ob.type == 'admin':
...
def adminpage(request):
    def adminpage(request):
        ob = report_table.objects.all().order_by("date")[:100]  # Fetch only first 1000 records
        ...
        dataset = []
        for i in dates:
            ob_filtered = report_table.objects.filter(date=i)
            dataset.append({"Date": i, "Value": len(ob_filtered)})
...
def  view_details(request):
    ...
    if reports_sorted:
        ...
        eports = report_table.objects.filter(disease=reports_sorted[0]['dis'], date=datetime.today())
